...
def init_db():
    ...
    # Forecast log — stores all 3 model forecasts with timestamps
    # So we can see how accurate models are 4 days out vs 1 day out
    c.execute("""CREATE TABLE IF NOT EXISTS forecast_log (
        id SERIAL PRIMARY KEY,
        logged_at_utc TEXT,        -- exact UTC timestamp when logged
        logged_at_est TEXT,        -- same in EST for easy reading
        city TEXT,
        target_date TEXT,          -- date the forecast is for
        days_until_resolution INT, -- how many days until market resolves
        gfs_temp REAL,             -- GFS model forecast
        ukmo_temp REAL,            -- UKMO model forecast  
        mf_temp REAL,              -- MeteoFrance model forecast
        consensus_temp REAL,       -- average of available models
        spread REAL,               -- max - min across models
        unit TEXT,                 -- C or F
